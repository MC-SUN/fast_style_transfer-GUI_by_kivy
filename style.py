...
CONTENT_WEIGHT = 7.5e0  # 内容权重
STYLE_WEIGHT = 1e2  # 风格权重
TV_WEIGHT = 2e2  # 全变差，使最终图像更真!!!

LEARNING_RATE = 1e-3  # 学习率!!!
NUM_EPOCHS = 2  # !!!
CHECKPOINT_DIR = 'checkpoints'  # 检查点路径
CHECKPOINT_ITERATIONS = 2000  # 迭代次数
VGG_PATH = 'Assets/data/imagenet-vgg-verydeep-19.mat'  # 训练网络
TRAIN_PATH = 'data/train2014'  # 训练集！
BATCH_SIZE = 4  # 一次迭代多少张
DEVICE = '/gpu:0'  # gpu进行训练
FRAC_GPU = 1  # !!!
'''命令行参数'''
...
def main():
	...
	kwargs = {
		...
		"batch_size": options.batch_size,  # 批处理。一组多少张，itertions=epoch/batch即迭代一轮的迭代次数， epochs*(epoch/batch)即为总迭代次数
	...
	for preds, losses, i, epoch in optimize(*args, **kwargs):  # 迭代!!!
		...
		if options.test:
			...
			if not options.slow:
				...
				save_img(preds_path, preds.reshape((512,512,3)))###＃131 错误save_img(preds_path,img),I found a fix for this. Replace img with preds.reshape((512,512,3))
