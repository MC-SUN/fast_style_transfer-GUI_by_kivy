...
def ffwd(data_in, paths_out, checkpoint_dir, device_t='/gpu:0', batch_size=4):
	print(data_in)#['image/content1.jpg']
	print(type(data_in))#<class 'list'>
	assert len(paths_out) > 0
	is_paths = type(data_in[0]) == str
	if is_paths:
		assert len(data_in) == len(paths_out)
		img_shape = get_img(data_in[0]).shape#HWC
		print(img_shape)
	else:
	...
	with g.as_default(), g.device(device_t), \
		...
		if os.path.isdir(checkpoint_dir):
			ckpt = tf.train.get_checkpoint_state(checkpoint_dir)
			if ckpt and ckpt.model_checkpoint_path:
				saver.restore(sess, ckpt.model_checkpoint_path)  # 加载预训练模型
			else:
				raise Exception("No checkpoint found...")
		else:
			saver.restore(sess, checkpoint_dir)
		...
		for i in range(num_iters):
			...
			for j, path_out in enumerate(curr_batch_out):
				save_img(path_out, _preds[j])  # 得到风格转换后图像
...
def main():
	...
	if not os.path.isdir(opts.in_path):#单个图片
		if os.path.exists(opts.out_path) and os.path.isdir(opts.out_path):#out_path是文件夹
			out_path = \
				os.path.join(opts.out_path, os.path.basename(opts.in_path))#xx/xx.jpg
		else:
			out_path = opts.out_path#可以自定义name:xx/xx.jpg

		print("!!!!!!"+opts.in_path,out_path, opts.checkpoint_dir,opts.device)
